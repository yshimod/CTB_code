...
foreach i of local levels {
    ...
    local DENOM2prsnt = " ( {beta=`TMPbeta'} * delta ^k ) ^ ( 1.0        / ( rho - 1 ) ) "
    local DENOM2      = " (                    delta ^k ) ^ ( 1.0        / ( rho - 1 ) ) "

    local NUMERprsnt = " ( {beta=`TMPbeta'} * delta ^k * pratio ) ^ ( 1.0 / ( rho - 1 ) ) "
    local NUMER      = " (                    delta ^k * pratio ) ^ ( 1.0 / ( rho - 1 ) ) "
...
save "base_estimated_M1.dta", replace
